## Welcome
Welcome to the notebook for model $0$ of our final project. This model was not a part of the three tries, but serves as a benchmark model based only on the tabular data.
...
nutrients_names <- colnames(food_train)[-(1:8)] # To be used later for nutrient feature extraction
...
rec <- recipe(category ~., data=train_ready) %>% prep(train_ready)
...
set.seed(42)
mod <- boost_tree(mode="classification", engine="xgboost", trees=100,
...
0.60*score_comparison(mod_acc, baseline_acc)
